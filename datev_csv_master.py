...
if uploaded_file is not None:
  ...
  def get_table_download_link(df, filename, link_label):
    ...
    towrite.seek(0)  # zurück zum Start des Streams
    b64 = base64.b64encode(towrite.read()).decode()  # Codieren Sie die Daten in Base64
  ...
  st.title(f"Umsatz gesamt: {total_gesamt_euro}")
  #st.divider()
  #KPI´s
...
# Nachdem Sie die Pivot-Tabelle erstellt haben, fügen Sie den Download-Link hinzu:
  ...
  #st.markdown(get_table_download_link(stb_umstz, "pivot_data.csv", "Download Pivot Daten als CSV"), unsafe_allow_html=True)
